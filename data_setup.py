...
def data(wavenumber=True):
    ...
    n2 = len(wavelength_centre)
    wavelength_bins = np.zeros(n2+1)
    wavelength_err = np.zeros(n2)

    wavelength_bins[0] = wavelength_centre[0] - (0.5 * wavelength_width[0])
    for i in range(n2):
        wavelength_bins[i+1] = wavelength_centre[i] + (0.5 * wavelength_width[i])
    ...
    if "line" in parameters:
        ...
        integral_dict, x_full = noniso.tau(p0)
