    ...
    **hist2d_kwargs
    ...
    if range is None:
        if "extents" in hist2d_kwargs:
            ...
            if np.any(m):
                raise ValueError(("It looks like the parameter(s) in column(s) {0} have no dynamic range. Please provide a `range` argument.").format(", ".join(map("{0}".format, np.arange(len(m))[m]))))
    ...
    if reverse:
        ...
        lbdim = 0.6 * factor   # size of left/bottom margin
        trdim = 0.2 * factor   # size of top/right margin
    whspace = 0.05         # w/hspace size
    plotdim = factor * K + factor * K * whspace
    ...
    for i, x in enumerate(xs):
        ...
        ### Define sizes
        tick_label_size = K + 8   # size of the parameter ticks (numbers)
        tick_size = K * 0.5       # size of the ticks in each squared plot
        font_size = 22            # size of the parameter labels
        line_width = 2.5          # width of median and quantile histogram lines

        # Plot the histograms.
        if smooth1d is None:
            n, _, _ = ax.hist(x, bins=bins[i], weights=weights, range=np.sort(range[i]), facecolor=fill_color, density=True, **hist_kwargs)
        else:
        ...
        if truths is not None and truths[i] is not None:
            ax.axvline(truths[i][0], color=sigma_color, linestyle='-', linewidth=line_width)
            ax.axvline(truths[i][0] + truths[i][1], color=sigma_color, linestyle=':', linewidth=line_width)
            ax.axvline(truths[i][0] - truths[i][2], color=sigma_color, linestyle=':', linewidth=line_width)
            ax.tick_params(axis='both', right=False, labelright=False, direction='in', length=tick_size)
        ...
        if ini_guess is not None and ini_guess[i] is not None:
            ...
            ax2 = ax.twinx()
            ax2.plot(x_gauss, norm.pdf(x_gauss, mu, sigma), color='k', linestyle='-', linewidth=line_width)
            ax2.tick_params(axis='both', right=False, labelright=False, direction='in', length=tick_size)
        ...
        for j, y in enumerate(xs):
            ...
            hist2d(y, x, ax=ax, range=[range2[j], range2[i]], weights=weights, color=fill_color, smooth=smooth, bins=[bins[j], bins[i]], **hist2d_kwargs)
            ...
            if i < K - 1:
                ...
                [l.set_rotation(45) for l in ax.get_xticklabels()]
                if labels is not None:
                    ax.set_xlabel(labels[j], fontsize=font_size, zorder=1000)
                    if reverse:
                        ax.xaxis.set_label_coords(0.5, 1.0) #(0.5, 1.4)
                    else:
                        ax.xaxis.set_label_coords(0.5, -0.25 -0.01*K) #(0.5, -0.3)
            ...
            ax.tick_params(axis='x', which='major', direction='in', length=tick_size, labelsize=tick_label_size, pad=6)
            ax.tick_params(axis='y', which='major', direction='in', length=tick_size, labelsize=tick_label_size, pad=3)

        ax.xaxis.set_label_coords(0.5, -0.25 -0.01*K) #(0.5, -0.3)
        ax.tick_params(axis='both', which='major', direction='in', length=tick_size, labelsize=tick_label_size, pad=6)

#    if K == 2:
#        ax = plt.subplot2grid((4*K, 4*K), (0, 5), colspan=3, rowspan=3)
#    else:
#        ax = plt.subplot2grid((2*K, 2*K), (0, K+1), colspan=K-1, rowspan=K-1)
...
#    ax.plot(xbin, ydata, 'ro', mew=mew1, markersize=msize, zorder=1000)
#    label_size2 = 22*K/3 + 20
#    text_size = label_size2 - 20 #22*K/4 + 20
...
#    ax.xaxis.set_major_formatter(ScalarFormatter(useMathText=use_math_text))
#    ax.yaxis.set_major_formatter(ScalarFormatter(useMathText=use_math_text))
#    tick_label_size2 = 7*K + 14
#    tick_size2 = 4*K
#    ax.tick_params(axis='both', which='major', direction='in', length=tick_size2, labelsize=tick_label_size2, pad=2*K)
#    ax.set_xlabel(r'Wavelength ($\mu$m)', fontsize=label_size2, labelpad=4*K)
#    ax.set_ylabel(r'($R$/$R_{\rm star}$)$^2$ (\%)', fontsize=label_size2, labelpad=4*K)
    #ax.set_xlabel(r'\textbf{Wavelength (}\boldmath{$\mu$}\textbf{m)}', fontsize=label_size2, labelpad=100)
    #ax.set_ylabel(r'\textbf{(}\boldmath{$R$/$R_{\rm star}$}\textbf{)}\boldmath{$^2$} \textbf{ (\%)}', fontsize=label_size2, labelpad=100)
...
def quantile(x, q, weights=None):
    ...
    weights : Optional[array_like[nsamples,]]
        An optional weight corresponding to each sample. These Returns
    -------
    quantiles : array_like[nquantiles,]
        The sample quantiles computed at ``q``. Raises
    ------
    ValueError
...
def hist2d(x, y, bins=20, range=None, weights=None, levels=None, smooth=None, ax=None, color=None, plot_datapoints=True, plot_density=True, plot_contours=True, no_fill_contours=False, fill_contours=False, contour_kwargs=None, contourf_kwargs=None, data_kwargs=None, **kwargs):
    ...
    no_fill_contours : bool
        Add no filling at all to the contours (unlike setting ``fill_contours=False``, which still adds a white fill at the densest points).
    ...
    if range is None:
        if "extent" in kwargs:
            logging.warn("Deprecated keyword argument 'extent'. Use 'range' instead.")
            range = kwargs["extent"]
        else:
    ...
    try:
        ...
        raise ValueError("It looks like at least one of your sample columns have no dynamic range. You could try using the 'range' argument.")
